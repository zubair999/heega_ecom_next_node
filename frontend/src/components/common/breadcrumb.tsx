...
const BreadcrumbItem: React.FC<Props> = ({ children, ...props }) => {
	return (
		<li
			className="text-body px-1 transition duration-200 ease-in first:ps-0 last:pe-0 hover:text-heading text-sm font-bold"
			{...props}
		>
			{children}
		</li>
...
export const BreadcrumbItems = (props: any) => {
	...
	return (
		<div className="flex items-left flex-col">
			<ol className="flex items-center justify-left w-full overflow-hidden">{children}</ol>
		</div>
	);
};
