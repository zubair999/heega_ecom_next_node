	...
	return (
		<div className="megaMenu shadow-header bg-gray-200 absolute -start-20 xl:start-0 opacity-0 invisible group-hover:opacity-100 group-hover:visible">
			<div className="grid grid-cols-2">
				{columns?.map((column) => (
					<ul
						className="even:bg-gray-150 2xl:pb-8 pt-2 2xl:pt-7"
						key={column.id}
					>
							...
							<React.Fragment key={columnItem.id}>
								<li className="mb-1">
									<Link
										...
										className="block text-sm py-1.5 text-heading font-semibold px-5 xl:px-4 2xl:px-10 hover:text-heading capitalize"
									>
										{t(columnItem.label)}
									</Link>
								</li>
								{columnItem?.columnItemItems?.map((item: any) => (
									...
									>
										<Link
											href={item.path}
											className="text-body text-xs block py-1 px-5 xl:px-4 2xl:px-10 hover:text-heading  capitalize"
										>
											{t(item.label)}
										</Link>
									</li>
